...
See [`EVAL.md`](EVAL.md) for the current dated run. Latest (2026-08-06): **16/16 scenarios passed** (controlled 13/13, end-to-end 3/3, 100% JSON/procedure success). Contradiction detection is now deterministic (`CONTRADICTION_HINT` covers frequency / scope / process incompatibilities and does not false-flag a consistent case). `EVAL.md` also reports a phase-level **performance** section (cold vs warm).

## Performance & optimization
The interactive interview was tuned (`bench.py`): evidence is retrieved **once** at startup (adaptive search) and reused; live `DATA_SIGNALS` is computed **once per topic** and reused; the AI gets a **compact structured state** (`DISCOVERY_NEXT_V2`) rather than the full transcript; and persistence is **batched** (`MERGE` per turn + one `VARIANT`/`FLATTEN` artifact write), which dropped summary-time persistence ~4.1s→~0.6s (summary p50 ~13.6s→~10.0s) and cut per-turn round-trips. Per-turn latency remains dominated by the `AI_COMPLETE` call (`mistral-large2`, ~8–12s, region-variable), so the aggressive next-question p50≤5s target is **not met** with the quality-passing model; a benchmarked faster model (`llama3.1-8b`, ~3× faster) failed the ≥9/13 quality gate and was rejected, and `claude-3-5-haiku` is unavailable in-region. The model is centralized in `MODEL()` for a one-line swap. Latency targets are treated as goals and reported honestly, not claimed as achieved.
...
- The app runs under a least-privilege role (`NOMY_APP_ROLE`, `sql/08_app_role.sql`); **`ACCOUNTADMIN` is not required** for normal use.
